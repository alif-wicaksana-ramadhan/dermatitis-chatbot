...
# Path ke file Excel
file_path = "dataset/data_da.xlsx"
output_path = "dataset/data_da_cleaned.csv"
...
selected_columns = [
    ...
    df.columns[10],  # Kolom ke-7
    df.columns[19],  # Kolom ke-12
    df.columns[20],  # Kolom ke-13
    df.columns[21],  # Kolom ke-14
    df.columns[22],  # Kolom ke-15
]

column_renaming = {
    "usia__pasien2": "patient_age",
...
data_cleaned.loc[:, "keluhan__utama_dan__onset"] = (
    data_cleaned["keluhan__utama_dan__onset"]
    + "\n"
    + data_cleaned["kriteria__mayor"]
    + "\n"
    + data_cleaned["kriteria__minor"]
)
data_cleaned = data_cleaned.drop(columns=["kriteria__mayor", "kriteria__minor"])
data_cleaned = data_cleaned.rename(columns=column_renaming)
data_cleaned.loc[:, "classname"] = "dermatitis atopic"
...
train_df.to_csv("dataset/data_da_train_cleaned.csv", index=False)
test_df.to_csv("dataset/data_da_test_cleaned.csv", index=False)
